...
Settings) build on top of it. `00000000000012_home_curation.sql` adds a
capped, admin-curated `is_featured` flag on listings (cap stored in
`app_settings.featured_stays_max_slots`, default 25) and a
`listing_booking_counts()` aggregate used to rank "Popular stays" by actual
bookings and to pick each destination's most-booked photo. **To stand up a
brand new project from zero** — e.g. after a project is deleted — run every
...
blank placeholder rows for those.
