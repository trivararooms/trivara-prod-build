...
describe('bookingService.create', () => {
  ...
  it('refuses to create a booking when Razorpay is not configured, without inserting anything', async () => {
    ...
    // Exactly 3 `.from()` calls (profiles, listings, bookings-conflict-check) -
    // no fourth call for an insert.
...
describe('bookingService.getStats', () => {
  it('aggregates confirmed/completed counts and derives 85% net earnings', async () => {
    mockFrom.mockReturnValueOnce(createBuilder({
      data: [
    ...
    expect(stats.totalEarnings).toBeCloseTo(2000 * 0.85);
    expect(stats.pendingEarnings).toBeCloseTo(1000 * 0.85);
  });
});
